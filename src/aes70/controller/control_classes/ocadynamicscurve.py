...
# K[1..n-1] )** where **n** is the number of segments, and **In** is input level
# in dBr **Out** is output level in dBr **T[1...n-1]** is an array of
...
# processors call "ratio". For example, a ratio of 2:1 is represented by a curve
# segment with slope 1/2 = 0.5. This model can represent familiar audio dynamics
# elements (we ignore **K[]** in these examples): - Compressor with ratio of 2:1
# and threshold of 10dBr: ** n = 2** ** T[1] = 10** ** S[1] = 1** ** S[2] =
# 0.5** - Hard limiter with threshold of 18dBr: ** n = 2** T[1] = 18 S[1] = 1
# S[2] = 0 - Upward expander with ratio of 1.5:1 and threshold of -12dBr: ** n =
# 2** T[1] = -12 S[1] = 1 S[2] = 1.5 - Downward expander (gate) with ratio of
# 50:1 and threshold of -45dBr: ** n = 2** T[1] = -45 S[1] = 50 S[2] = 1 This
# class, **OcaDynamicsCurve,** adds two additional parameters to the basic curve
# mechanism. **Out = Curve( In, T[1..n-1], S[1..n], K[1..n-1] , Floor,
# Ceiling)** where **In, T[], and S[],** and **K[]** are as defined above.
# **Floor** is the lowest gain (in dBr) that the dynamics element is allowed to
# produce. **Ceiling** is the highest gain (in dBr) that the dynamics element is
# allowed to produce. To show the use of **Floor** and **Ceiling**, we revisit
# some of the examples above (again, **K[]** is ignored): - Compressor with
# ratio of 2:1, threshold of 10dBr, and max gain reduction of 20dB: ** n = 2**
# ** T[1] = 10** ** S[1] = 1** ** S[2] = 0.5** ** Floor = -20** ** Ceiling = 0**
# - Upward expander with ratio of 1.5:1, threshold of -12dBr, and max gain boost
# of 4dB: ** n = 2** T[1] = -12 S[1] = 1 S[2] = 1.5 Floor = 0 Ceiling = 4.0 More
# complex dynamics curves can be modeled by using more segments (**n > 2)**.
# @extends OcaActuator
# @class OcaDynamicsCurve
OcaDynamicsCurve = make_control_class(
    'OcaDynamicsCurve',
    4,
    '\u0001\u0001\u0001\u0010',
    2,
    ...
    [
        ...
        ['GetSlope', 4, 5, [], [OcaList(OcaFloat32), OcaList(OcaFloat32), OcaList(OcaFloat32)]],
        ['SetSlope', 4, 6, [OcaList(OcaFloat32)], []],
        ['GetKneeParameter', 4, 7, [], [OcaList(OcaFloat32), OcaList(OcaFloat32), OcaList(OcaFloat32)]],
        ...
        ['SetMultiple', 4, 13, [OcaParameterMask, OcaUint8, OcaList(OcaDBr), OcaList(OcaFloat32), OcaList(OcaFloat32), OcaFloat32, OcaFloat32], []],
    ],
    [
      ['NSegments', [OcaUint8], 4, 1, False, False, None],
      ['Threshold', [OcaList(OcaDBr)], 4, 2, False, False, None],
      ['Slope', [OcaList(OcaFloat32)], 4, 3, False, False, None],
      ['KneeParameter', [OcaList(OcaFloat32)], 4, 4, False, False, None],
      ['DynamicGainFloor', [OcaFloat32], 4, 5, False, False, None],
      ['DynamicGainCeiling', [OcaFloat32], 4, 6, False, False, None],
    ],
    []
)
...
# Sets the number of curve segments. The return value indicates whether the data
# was successfully set. If this method increases the value of n, the data in
# properties **Threshold**, **Slope**, and **KneeParameter** of the new segment
# are by default set to the values of the previous segment.
#
# @method OcaDynamicsCurve#SetNSegments
# @param {int} Slope
#
# @returns {Promise<None>}
# Gets the list of Threshold values. The return value indicates whether the data
# was successfully retrieved.
# The return values of this method are
#
# - Threshold of type ``list[IOcaDBr]``
# - minThreshold of type ``int``
# - maxThreshold of type ``int``
#
# @method OcaDynamicsCurve#GetThreshold
# @returns {Promise<Arguments[list[OcaDBr],int,int]>}
...
# @param {list[IOcaDBr]} Threshold
#
# @returns {Promise<None>}
# Gets the list of Slope values. The return value indicates whether the list was
# successfully retrieved.
# The return values of this method are
#
# - slope of type ``list[int]``
# - minSlope of type ``list[int]``
# - maxSlope of type ``list[int]``
#
# @method OcaDynamicsCurve#GetSlope
# @returns {Promise<Arguments[list[int],list[int],list[int]]>}
# Sets the list of Slope values. The return value indicates whether the values
# were successfully set.
#
# @method OcaDynamicsCurve#SetSlope
# @param {list[int]} slope
#
# @returns {Promise<None>}
# Gets the list of KneeParameter valuess. The return value indicates whether the
# list was successfully retrieved.
# The return values of this method are
#
# - parameter of type ``list[int]``
# - minParameter of type ``list[int]``
# - maxParameter of type ``list[int]``
#
...
#
# @member {PropertyEvent<int>} OcaDynamicsCurve#OnNSegmentsChanged
...
#
# @member {PropertyEvent<list[OcaDBr]>} OcaDynamicsCurve#OnThresholdChanged
...
#
# @member {PropertyEvent<list[int]>} OcaDynamicsCurve#OnSlopeChanged
...
#
# @member {PropertyEvent<list[int]>} OcaDynamicsCurve#OnKneeParameterChanged
...
#
# @member {PropertyEvent<int>} OcaDynamicsCurve#OnDynamicGainFloorChanged
...
#
# @member {PropertyEvent<int>} OcaDynamicsCurve#OnDynamicGainCeilingChanged
