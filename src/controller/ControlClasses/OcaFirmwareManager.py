...
from ....aes70.OCP1.OcaComponent import OcaComponent
...
from ....aes70.OCP1.OcaVersion import OcaVersion
...
# controllers. This firmware manager offers a generic interface for updating OCA
# devices. The actual robustness of the update process is left up to the
# implementer. The interface allows for any of: - Fully transactional based
# uploads (i.e. only committing to the newly uploaded images after all component
# uploads have succeeded, and reverting back to the old images if any step
# fails) - Partly transactional based uploads (i.e. committing to a newly
...
OcaFirmwareManager = make_control_class(
  'OcaFirmwareManager',
  3,
  '\u0001\u0003\u0003',
  2,
  ...
  [
    ...
    ['BeginPassiveComponentUpdate', 3, 7, [OcaComponent, OcaBlob, OcaString], []],
...
# @returns {Promise<list[OcaVersion]>}
#   A promise which resolves to a single value of type ``list[OcaVersion]``.
# Marks the start of the update process of an OCA device, meaning one or more
# components will be updated. If the method succeeds the device will be in state
# 'Updating'. One or more active or passive updates can then follow, after which
# the update process is ended by calling the '03m08 EndUpdateProcess' method.
# The return value indicates if starting the update process succeeded.
#
...
# The return value indicates whether the data is correctly received and the data
# is not out of order.
#
...
# be used for any component that can be passively updated, i.e. where the device
# requests the actual software/firmware image from an external server. In the
# function the component type, details of the server and the filename of the
# file containing the component software/firmware image needs to be passed. The
# device will try to retrieve the new software/firmware image from the server
# and update its component using this image. The actual method for retrieving
# the image (e.g. TFTP) and the underlying update technique (e.g. netflash)
# depend on the implementation and may differ between components. Just the
...
# class, component numbers are of datatype** OcaEnum,** rather than the previous
# **OcaUint16.**
#
# @member {PropertyEvent<list[OcaVersion]>} OcaFirmwareManager#OnComponentVersionsChanged
